Source URL: Internal discussion
...
Authenticated SaaS web applications sometimes need more than a static browser-only dashboard. When the application must integrate with a separate OAuth2 or OIDC authorization service, the web app needs server-side responsibilities that a static React + Vite SPA should not own.

The current authentication direction is a separate self-hosted auth service using Better Auth as the main library. That auth service owns login, SSO, credential handling, identity provider integration, consent, OAuth/OIDC issuer metadata, token issuance, JWKS, token revocation, and account-level authentication policy.

The SaaS web app should not handle credential login directly. It should rely on the auth service through OAuth2/OIDC and manage only the application session and product authorization boundary.

React + Vite remains a strong default for static authenticated dashboards that consume a separate backend API and have no SSR, OAuth callback, server function, or app-server session requirement. For OAuth2/OIDC-required SaaS apps, TanStack Start is the preferred default because it can act as a full-stack app server or Backend-for-Frontend while preserving React, TanStack Router, TanStack Query, TanStack Form, and TanStack Table as the application foundation.

## Decision

Use **TanStack Start** as the default web frontend stack for SaaS applications that require OAuth2 or OIDC authentication.

The standard stack name is:

```text
Web Stack: TanStack Start
```

This stack is the default for web apps that need any of the following:

- OAuth2 Authorization Code Flow with PKCE.
- OIDC Authorization Code Flow with PKCE.
- SSO through a separate auth service.
- Server-side OAuth/OIDC callback handling.
- Server-side code exchange.
- Server-managed app sessions.
- HttpOnly cookie sessions.
- SSR protected by server-side session checks.
- Server functions that require authentication and authorization.
- Backend-for-Frontend behavior.
- Full-stack resource ownership inside the web app server.

Better Auth is the current self-hosted auth service standard, but this web stack remains compatible with any standards-compliant OAuth2/OIDC provider.

For new SaaS products, the default resource ownership profile is **TanStack Start as full-stack app server**. The BFF profile is used when a standalone backend API already exists or must remain authoritative.

## Scope

This stack covers authenticated SaaS web applications where the web app participates in OAuth2/OIDC flows and has a production server runtime.

In scope:

- Product dashboards with OAuth2/OIDC login.
- SaaS apps that need server-managed sessions.
- SaaS apps that need protected server functions.
- Apps that directly own their product database from the web server.
- Apps that call an existing backend API through a server-side BFF layer.
- Apps that need SSR, route-level loading, or app-server-controlled redirects.
- Tenant/workspace-aware SaaS administration surfaces.

Out of scope:

- Public marketing and SEO landing sites. Use the Astro landing stack.
- Static API-only SPA dashboards with no server runtime. Use the React + Vite dashboard stack.
- Standalone public backend APIs, machine APIs, webhook-heavy systems, and worker-heavy services. Use the backend API stack unless the product intentionally consolidates them into the TanStack Start app server.
- Mobile apps. Use the React Native + Expo stack.

## Relationship To React + Vite Stack

React + Vite remains valid and should not be replaced for static dashboard use cases.
...
- The app needs HttpOnly cookie sessions.
...
The key difference is runtime ownership. React + Vite is static. TanStack Start has a production server runtime and is part of the authentication and resource boundary.
...
- Drizzle for database access when the app owns its database.
- PostgreSQL for durable SaaS data.
...
- Radix UI primitives through Shadcn.
- Sonner for toasts.
- Recharts through Shadcn chart patterns when charts are needed.
- `lucide-react` as the default icon library unless the design preset chooses another.
...
Required OAuth/OIDC properties:
...
The app should support explicit re-authentication or session refresh according to provider capability and product policy.

## Better Auth Provider Assumptions

Better Auth is the current self-hosted auth service standard.

The Better Auth service is expected to provide:
...
- Login page.
- Consent page when product or compliance requirements need consent.
...
Better Auth should use OAuth/OIDC provider capability plus JWT/JWKS support for provider-side token verification.

The TanStack Start app is not the credential-login app. It is an OAuth/OIDC client of the central auth service.

Provider compatibility rule:

- This stack can use another OAuth2/OIDC provider if it supports the same authorization code, issuer, JWKS, client registration, token, and revocation requirements.

## TanStack Start Responsibilities

The TanStack Start app owns application-level session and product access, not credential login.

Responsibilities:

- Initiate OAuth/OIDC login.
- Generate and validate `state`.
- Generate and use PKCE code verifier/challenge.
- Handle OAuth/OIDC callback route.
- Exchange authorization code on the server.
- Create and clear app sessions.
- Store provider tokens server-side only when needed.
- Resolve current user and tenant/workspace context.
- Protect SSR route loading.
- Protect server functions and server routes.
- Enforce product authorization and tenant isolation.
- Render authenticated app shell.
- Own product database access when using the full-stack profile.
- Call downstream backend APIs server-side when using the BFF profile.
...
Redis is mandatory from day one for TanStack Start OAuth/OIDC apps because live session invalidation, logout, callback state, PKCE verifier storage, CSRF state, rate limiting, and distributed runtime support are part of the authentication boundary.
...
- Do not store refresh tokens in `sessionStorage`.
...
- Database-backed live sessions are not the default for this stack; use Redis for live session state.
...
- Report generation.
- File processing.
- Image and document transformation.
- Email or notification fanout.
- Billing reconciliation.
- Webhook retry and delivery.
- Slow or retryable third-party API calls.
- AI or LLM generation.
- Large data sync jobs.
- Scheduled maintenance jobs.
- Any process expected to exceed the normal request latency budget.
- Any process that needs retry, backoff, deduplication, progress tracking, or durable failure handling.
...
- Never expose provider access tokens to browser JavaScript.
- Never expose provider refresh tokens to browser JavaScript.
...
- The runtime image must not include source files, tests, caches, local `.env` files, or public source maps unless explicitly required and reviewed.
- Use pinned Alpine, slim, distroless, or the smallest production-suitable image variants where available and compatible.
- Final app/worker runtime images must target below `200 MB`; exceeding this budget requires an ADR or documented implementation note with measured image size and justification.
- Follow the infrastructure Docker image size and final runtime dependency policy.
...
- In monorepos, copy only the package manifests needed by the TanStack Start app or use a minimal generated runtime `package.json` so unrelated workspace packages are not installed.
...
Reference Node-compatible runtime Dockerfile:
...
- TLS termination.
- HTTP to HTTPS redirect outside ACME challenge paths.
- Security headers.
- Compression.
- Request size limits.
- Proxying to the TanStack Start runtime.
- Optional edge rate limiting where useful.

TanStack Start runtime responsibilities:

- SSR.
- Server functions.
- OAuth/OIDC callback handling.
- App session handling.
- BullMQ job enqueueing for app-owned async workflows.
- Product service logic for the full-stack profile.
- Server-side backend API calls for the BFF profile.
...
- Scheduled maintenance jobs when owned by the app.
...
| Question | Decision |
| --- | --- |
| Default app session storage | Redis-backed live sessions from day one. PostgreSQL may store audit/session history, but Redis is the primary live session store. |
| OAuth/OIDC client implementation | Provider-neutral standard. Use a vetted OAuth/OIDC helper library or provider SDK for discovery, code exchange, token validation, and revocation. Hand-rolled OAuth/OIDC requires ADR and security review. |
| Tenant membership source | Auth service is the source of truth for identity. Product database is the source of truth for tenant membership, product RBAC, entitlements, subscription state, soft locks, and operator overrides. Better Auth organization data may bootstrap or mirror tenant identity, but product authorization resolves in the app database by default. |
| Redis requirement | Redis is mandatory from day one for sessions, callback state, PKCE verifier storage, CSRF state, rate limiting, distributed invalidation, and BullMQ queues. |
| Long-running work | BullMQ backed by Redis is mandatory. Server functions enqueue jobs and return quickly; workers process long-running work outside the request lifecycle. |
| OpenAPI for TanStack Start routes | Server functions are the default first-party web contract. OpenAPI is required only for public APIs, mobile clients, third-party integrations, cross-service contracts, or backend APIs consumed outside the TanStack Start app. |
| Deployment output | Docker Compose with Nginx gateway and the standard TanStack Start/Nitro-compatible production runtime is the default. Platform-specific adapters are allowed by ADR with documented runtime constraints, secret handling, session behavior, and rollback path. |
